...
func generateCert(orgs, dnsNames []string, commonName string) (*bytes.Buffer, *bytes.Buffer, *bytes.Buffer, error) {
	...
	ca := &x509.Certificate{
		...
		NotBefore:             time.Now(),
		NotAfter:              time.Now().AddDate(2, 0, 0), // expired in 1 year
		IsCA:                  true,
...
func main() {
	...
	var err error
	org := "cliufreever"
	caPEM, _, _, err := generateCert([]string{org}, dnsNames, commonName)
	if err != nil {
		panic(fmt.Sprintf("Failed to generate ca and certificate key pair: %v", err))
	}
